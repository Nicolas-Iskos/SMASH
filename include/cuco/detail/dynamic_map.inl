...
dynamic_map<ReductionOp, Key, Value, Scope, Allocator, submap_type>::dynamic_map(
  ...
  submaps_.push_back(
    ...
    new submap_type<ReductionOp, Key, Value, Scope, Allocator>{initial_capacity, empty_key_sentinel}});
...
template<typename ReductionOp, typename Key, typename Value, cuda::thread_scope Scope, typename Allocator,
         template<typename, typename, typename, cuda::thread_scope, typename> typename submap_type>
void dynamic_map<ReductionOp, Key, Value, Scope, Allocator, submap_type>::reserve(std::size_t n) {
  ...
  while(num_elements_remaining > 0) {
    ...
    // if the submap does not exist yet, create it
      ...
      submaps_.push_back(
        std::unique_ptr<submap_type<ReductionOp, Key, Value, Scope, Allocator>>{
        new submap_type<ReductionOp, Key, Value, Scope, Allocator>{submap_capacity, empty_key_sentinel_}});
...
void dynamic_map<ReductionOp, Key, Value, Scope, Allocator, submap_type>::insert(
...
void dynamic_map<ReductionOp, Key, Value, Scope, Allocator, submap_type>::find(
...
template <typename InputIt, typename OutputIt, 
          typename Hash, typename KeyEqual>
void dynamic_map<ReductionOp, Key, Value, Scope, Allocator, submap_type>::contains(
  InputIt first, InputIt last, OutputIt output_begin,
  Hash hash, KeyEqual key_equal) noexcept {
